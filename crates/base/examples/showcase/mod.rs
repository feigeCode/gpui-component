...
use gpui::{
    ...
    WindowOptions, actions, div, prelude::FluentBuilder as _, px, rgb_to_hsla, size,
};
    ...
    pub fn new(component: impl Into<String>, window: &mut Window, cx: &mut Context<Self>) -> Self {
        ...
        let input = cx.new(|cx| {
            ...
            state.set_editor_style(InputEditorStyle {
                foreground: rgb_to_hsla(example_rgb(0x171717)),
                muted_foreground: rgb_to_hsla(example_rgb(0x737373)),
                selection: gpui::hsla(0.6, 0.8, 0.7, 0.45),
                caret: rgb_to_hsla(example_rgb(0x171717)),
        ...
        let color_picker = cx.new(|cx| {
            ColorPickerState::new(window, cx).default_value(rgb_to_hsla(example_rgb(0x2563eb)))
        });
